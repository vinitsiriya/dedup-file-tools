...
def run_import_checksums(db_path, checksum_db_path, other_db_path):
    logging.info(f"[COMPARE][IMPORT] Importing checksums from {other_db_path} to {checksum_db_path}")
    ...
    logging.info(f"[COMPARE][MAIN] Rows to import from other DB: {len(rows)} rows")
    min_batch_size = 5000
    total_rows = len(rows)
    ...
    try:
        ...
        logging.info(f"[COMPARE][MAIN] All rows in main job's checksum_cache after import: {count} rows")
    finally:
        conn.close()
    logging.info(f"[COMPARE][MAIN] Imported {len(rows)} checksums from {other_db_path} into checksum_cache.")
    return 0
