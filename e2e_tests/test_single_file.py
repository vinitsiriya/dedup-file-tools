...
def test_single_file():
    ...
    run_quiet([python_exec, '-m', 'fs_copy_tool.main', 'add-file', '--job-dir', str(job_dir), '--file', str(src / 'file1.txt')], env)
    ...
    assert (dst / 'file1.txt').exists(), "file1.txt not copied"
    assert (dst / 'file1.txt').read_text(encoding='utf-8') == 'hello world'
