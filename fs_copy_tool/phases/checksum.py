...
def update_checksums(db_path, table, threads=4):
    ...
    def process_row(row):
        ...
        return True
    with tqdm(total=len(rows), desc=f"Checksumming {table}") as pbar:
...
def import_checksums_from_old_db(new_db_path, old_db_path, table):
    ...
    Shows a progress bar for user feedback.
    """
    with sqlite3.connect(old_db_path) as old_conn, sqlite3.connect(new_db_path) as new_conn:
        ...
        updates = 0
        with tqdm(total=len(all_rows), desc=f"Importing checksums to {table}") as pbar:
