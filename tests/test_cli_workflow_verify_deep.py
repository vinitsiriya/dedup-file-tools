...
def test_cli_workflow_verify_deep(tmp_path):
    ...
    # 5. Deep verify phase
    args = main.parse_args(["deep-verify", "--job-dir", str(job_dir), "--job-name", job_name])
    assert main.run_main_command(args) == 0

    # Check DB for deep verify results
    db_path = job_dir / f"{job_name}.db"
    with sqlite3.connect(db_path) as conn:
        for i in range(5):
            row = conn.execute(
                "SELECT verify_status FROM verification_deep_results WHERE relative_path LIKE ? ORDER BY timestamp DESC LIMIT 1",
                (f"%file{i}.txt",)
            ).fetchone()
            print(f"[DEBUG] DB deep verify_status for file{i}.txt: {row}")
            assert row and row[0] == 'ok'
    print("[INFO] Deep verify phase and DB status checks passed.")
