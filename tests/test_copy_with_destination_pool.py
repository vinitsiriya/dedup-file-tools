...
def test_copy_with_destination_pool(tmp_path):
    ...
    args = main.parse_args(["add-to-destination-index-pool", "--job-dir", str(job_dir), "--dst", str(dst_dir)])
    assert main.run_main_command(args) == 0
    # 5. Copy with pool deduplication
    args = main.parse_args(["copy", "--job-dir", str(job_dir), "--src", str(src_dir), "--dst", str(dst_dir)])
    assert main.run_main_command(args) == 0
    ...
    with sqlite3.connect(db_path) as conn:
        row1 = conn.execute("SELECT copy_status FROM source_files WHERE relative_path LIKE ?", ("%file1.txt",)).fetchone()
        row2 = conn.execute("SELECT copy_status FROM source_files WHERE relative_path LIKE ?", ("%file2.txt",)).fetchone()
