...
def test_import_checksums_from_other_db(tmp_path):
    ...
    # 1. Init both jobs
    args = main.parse_args(["init", "--job-dir", str(main_job)])
    assert main.run_main_command(args) == 0
    args = main.parse_args(["init", "--job-dir", str(other_job)])
    assert main.run_main_command(args) == 0

    # 2. Analyze and checksum in other_job (use source_files table)
    args = main.parse_args(["analyze", "--job-dir", str(other_job), "--src", str(src_dir)])
    assert main.run_main_command(args) == 0
    args = main.parse_args(["checksum", "--job-dir", str(other_job), "--table", "source_files"])
    assert main.run_main_command(args) == 0

    # 3. Import checksums from other_job into main_job
    args = main.parse_args(["import-checksums", "--job-dir", str(main_job), "--other-db", str(other_job / "copytool.db")])
    assert main.run_main_command(args) == 0
    ...
    args = main.parse_args(["import-checksums", "--job-dir", str(main_job), "--other-db", str(empty_db)])
